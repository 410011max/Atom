    ...
    for name, m in tqdm(list(model.named_modules())):
        ...
        if isinstance(m, LlamaMLP):
            ...
            m.q_proj = W8A8Linear.from_float(
                ...
                quantize_output=quantize_bmm_input, scales=scales[name + ".q_proj"] if scales else None)
            m.k_proj = W8A8Linear.from_float(
                m.k_proj, weight_quant=weight_quant, act_quant=act_quant, 
                quantize_output=quantize_bmm_input, scales=scales[name + ".k_proj"] if scales else None)
            m.v_proj = W8A8Linear.from_float(
                m.v_proj, weight_quant=weight_quant, act_quant=act_quant, 
                quantize_output=quantize_bmm_input, scales=scales[name + ".v_proj"] if scales else None)
